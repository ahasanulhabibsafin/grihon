...
import { Download, Plus, X, Image as ImageIcon, Github, Search, User, Lock, LogOut } from 'lucide-react';
    ...
    <div className="min-h-screen selection:bg-black selection:text-white">
        ...
        <div className="nike-container h-16 flex items-center justify-between">
          <div className="flex items-center gap-8">
            ...
            <div className="hidden md:flex gap-6 font-bold text-sm uppercase items-center">
              ...
              {isAdmin && <span className="bg-black text-white text-[10px] px-2 py-0.5 rounded tracking-widest font-black text-xs">ADMIN MODE</span>}
            </div>
          </div>
          <div className="flex items-center gap-6">
            <div className="flex items-center gap-4">
              ...
              {currentUser ? (
                <button onClick={handleLogout} className="p-1 hover:bg-gray-100 rounded-full transition-colors"><LogOut className="w-6 h-6" /></button>
              ) : (
                <button onClick={() => setIsLoginModalOpen(true)} className="p-1 hover:bg-gray-100 rounded-full transition-colors"><User className="w-6 h-6" /></button>
              )}
            </div>
          </div>
        </div>
      ...
      <footer className="bg-black text-white pt-20 pb-10">
        <div className="nike-container text-center">
          <p className="text-[10px] text-gray-500 uppercase font-bold">© 2026 Grihon visuals. All rights reserved.</p>
        </div>
      </footer>
        ...
        <AnimatePresence>
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
            ...
            <motion.div
              ...
              className="relative bg-white w-full max-w-xl rounded-[40px] overflow-hidden shadow-2xl p-10"
            >
            ...
            <form onSubmit={handleAddImage} className="flex flex-col gap-6">
              ...
              />
              <button type="submit" className="w-full bg-black text-white py-5 rounded-full font-black uppercase">DEPLOY</button>
            </form>
            </motion.div>
          </div>
        </AnimatePresence>
      ...
      {isLoginModalOpen && (
        <AnimatePresence>
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
            ...
            >
            <h3 className="text-3xl font-black tracking-tighter uppercase mb-8">Member Access</h3>
            <form onSubmit={handleLogin} className="flex flex-col gap-6">
              ...
              <input 
                ...
                required
              />
              <button type="submit" className="w-full bg-black text-white py-5 rounded-full font-black uppercase">AUTHENTICATE</button>
            </form>
            </motion.div>
          </div>
        </AnimatePresence>
...
function ImageCard({ img, isAdmin, onDelete, onDownload }: { 
  ...
  return (
    <div className="group relative bg-[#F1F1F1] overflow-hidden cursor-pointer rounded-xl">
      <img src={img.url} alt="Gallery" className="w-full h-auto object-contain transition-transform duration-700 group-hover:scale-105" referrerPolicy="no-referrer" loading="lazy" />
      <div className="absolute inset-0 bg-black/10 opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex flex-col justify-end p-8 text-white">
        {isAdmin && <button onClick={(e) => { e.stopPropagation(); onDelete(img.id); }} className="absolute top-6 right-6 bg-white/20 backdrop-blur-md p-2 rounded-full hover:bg-red-500 transition-colors"><X className="w-4 h-4" /></button>}
        <div className="flex justify-between items-end">
          ...
          <button onClick={(e) => { e.stopPropagation(); onDownload(img.url, img.id); }} className="bg-white text-black px-4 py-2 rounded-full font-bold flex items-center gap-2"><Download className="w-4 h-4" /> Download</button>
        </div>
      </div>
    </div>
  );
